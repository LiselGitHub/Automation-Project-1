...
describe('This is first test suite, Lisel Kaup', () => {
    // 13.07.2024
    it('User can submit data only when valid mandatory values are added', () => {
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('#firstName').type('Lis')
        cy.get('#lastName').type('Ka')
        cy.get('input[name="password"]').type('Uussala321')
        cy.get('[name="confirm"]').type('Uussala321')
    ...
    it('User can use only same both first and validation passwords', () => {
        ...
        cy.get('input[name="password"]').type('Password123123')
   ...
   it('User cannot submit data when phone number is absent', () => {
       ...
       // Assert that submit button is not enabled and that successful message is not visible
        cy.get('#username').type('johnDoe')
        cy.get('[data-testid="phoneNumberTestId"]').type('1020340')
        cy.get("input[name='password']").type('Uussala321')
        cy.get('[name="confirm"]').type('Uussala321')
